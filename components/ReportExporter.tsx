...
export default function ReportExporter({ analysis }: { analysis: FullBlastRadiusAnalysis | null }) {
  ...
  const generateReport = (): string => {
    ...
    md += `**Sources Read**: ${analysis.researchSummary.totalSourcesFetched} (${analysis.researchSummary.unlockedSourceCount} via unlocker, ${analysis.researchSummary.cacheHits} cached)\n\n---\n\n`;
    for (const r of analysis.reports) {
      const { dependency, riskLevel, overallRiskScore, breakingChanges, research, sources } = r;
      ...
      for (const s of sources) {
        md += `  - [${s.sourceType}] [${s.title}](${s.sourceUrl}) — ${s.extractedClaims.length} claim(s), via ${s.transport}\n`;
      }
      md += `\n`;
      if (breakingChanges.length === 0) {
        // Absence of evidence is reported as such — never as safety.
        md += research.sourcesFetched === 0
          ? `> ⚠️ No sources could be read for this package. This is **not** evidence that the upgrade is safe.\n\n`
    ...
    md += `*Generated by Upgrade Intelligence — every finding above is quoted from the source it links to.*\n`;
    return md;
  };
